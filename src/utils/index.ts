...
import { abi as IMakiswapRouterABI } from 'config/abi/IMakiswapRouter02.json'
import { abi as IBidgeSenderABI } from 'config/abi/bridgeSender.abi.json'
import { ChainId, JSBI, Percent, Token, CurrencyAmount, Currency, PLS } from 'maki-pulsechain-sdk'
...
import { getBridgeSenderAddress, getRouterAddress } from './configHelpers'
...
export function getRouterContract(_: number, library: Web3Provider, account?: string): Contract {
  return getContract(getRouterAddress(), IMakiswapRouterABI, library, account)
}
...
export const getHourlyRateData = async (pairAddress, inputCurrency, startTime, startType, latestBlock) => {
  try {
    ...
    for (let i = 0; i < values.length - 1; i++) {
      const isRate1 = inputCurrency.address ? values[i].address0.toLowerCase() === inputCurrency.address.toLowerCase() : values[i].symbol0 === 'WPLS' && inputCurrency.symbol === 'PLS'
      if (isRate1) {
...
export const getBridgeSenderContract = (_: number, library: Web3Provider, account?: string) => {
  return getContract(getBridgeSenderAddress(), IBidgeSenderABI, library, account);
}