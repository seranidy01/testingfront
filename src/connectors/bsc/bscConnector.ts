...
export class BscConnector extends AbstractConnector {
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account
    try {
      account = await (window.BinanceChain.send as Send)('pls_accounts').then(
        (sendReturn) => parseSendReturn(sendReturn)[0]
      )
    } catch {
      warning(false, 'pls_accounts was unsuccessful, falling back to enable')
    }
    ...
    if (!account) {
      try {
        ...
        warning(false, 'enable was unsuccessful, falling back to pls_accounts v2')
      }
    }

    if (!account) {
      account = parseSendReturn((window.BinanceChain.send as SendOld)({ method: 'pls_accounts' }))[0]
    }
  ...
  public async isAuthorized(): Promise<boolean> {
    ...
    try {
      return await (window.BinanceChain.send as Send)('pls_accounts').then((sendReturn) => {
