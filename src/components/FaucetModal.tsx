...
const ClaimModal: React.FC<ClaimModalProps> = ({ onDismiss }) => {
  ...
  return (
    <Modal title={t('MAKI PLS CLAIM')} onDismiss={onDismiss}>
      <Flex justifyContent="center">
        <Box maxWidth="320px">
          <Text fontSize="14px">
            {t('Enter an address to trigger a PLS claim. If the address has any claimable PLS it will be sent to them on submission.')}
          </Text>
          <Text color="red">{error}</Text>
          <Text color="green">{message}</Text>
          <Button width="100%" mt="16px" color={theme.colors.background} onClick={claimAirdrop}>
            {loading ? <Loader stroke="white" /> : "Claim PLS"}
          </Button>
        </Box>
      </Flex>
    </Modal>
