...
import { ChainId, Currency, currencyEquals, PLS, Token } from 'maki-pulsechain-sdk'
  ...
  return (
    <AutoColumn gap="md">
      ...
      <AutoRow gap="auto">
        <BaseWrapper
          onClick={() => {
            if (!selectedCurrency || !currencyEquals(selectedCurrency, PLS)) {
              onSelect(PLS)
            }
          }}
          disable={selectedCurrency === PLS}
        >
          <CurrencyLogo currency={PLS} style={{ marginRight: 8 }} />
          <Text>BNB</Text>
        </BaseWrapper>
