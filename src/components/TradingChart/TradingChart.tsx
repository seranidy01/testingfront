...
import { Currency, Token, PLS, WPLS, ChainId } from 'maki-pulsechain-sdk'
...
import { CandlePeriod, NumericalCandlestickDatum } from 'config/types'
...
const CandlePeriodButton = styled.div`
  ...
  &:hover {
    background-color: ${({ theme }) => darken(0.05, theme.colors.navBackground)};
  }
...
export default function TradingChart({ inputCurrency, outputCurrency, minWidth, minHeight, backgroundColor }: ChartProps) {
  ...
  const chartWidth = Math.min(width - 16 - 16, minWidth ?? 550)
  const chartHeight = minHeight ?? 300
  ...
  return (
    <div style={{ padding: '1rem 2rem' }}>
      <FlexColumnWrapper>
        ...
        <ChartWrapper hasData={hasData} style={{ backgroundColor: backgroundColor ?? 'transparent' }}>
