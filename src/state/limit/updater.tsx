...
import { 
  filterOrdersFromPair,
  findOpenOrdersFromTokens,
...
export default function LimitUpdater() {
  ...
  useEffect(() => {
    if(contract && status === 'succeeded'){
      findOpenOrdersFromTokens(contract, tokens ?? ([] as any))
      .then( openOrders => {
        console.log('openOrders', openOrders)
         const filteredOrders = filterOrdersFromPair(openOrders, currencies?.INPUT, currencies?.OUTPUT)
         dispatch(updatePairOrders(filteredOrders))
         dispatch(updateStatus({
           status: undefined
         }))
      })
      .catch( e => console.error('openOrders', e))

    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tokens, contract, status, currencies?.INPUT?.symbol, currencies?.OUTPUT?.symbol, dispatch])
