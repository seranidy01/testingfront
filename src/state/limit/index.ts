...
import { LIMIT_INTERFACE } from 'config/abi'
import { getProviderOrSigner, isAddress } from 'utils'
import { LimitOrderTransactionReceipt, TokenMap } from '@types'
...
function getSwapType(token0: IToken | Token, token1: IToken | Token) {
  if (token0.symbol !== "WPLS" && token1.symbol !== "WPLS") { return EOrderType.TOKEN_TO_TOKEN; }
  if (token0.symbol === "WPLS") { return EOrderType.BUY; }
  return EOrderType.SELL;
}

export async function createOrder(
  accountAddress: string, token0: IToken | Token, token1: IToken | Token, amount0: number, amount1: number, gasPrice: number,
  feeStake: BigNumber, feeExecutor: BigNumber, contractAddress: string, provider: providers.Web3Provider
): Promise<LimitOrderTransactionReceipt> {
  if (!provider || !isAddress(accountAddress)) { return { id: -1,  transactionReceipt: undefined} }
  ...
  const gaslimitFn = await contract.estimateGas.createOrder(type, token0.address, token1.address, amountInFn, amountOutFn, feeExecutor, {
    value, gasPrice: gasGwei, gasLimit: BigNumber.from(330000)
  });
  // call contract
  const tx = await contract.createOrder(type, token0.address, token1.address, amountInFn, amountOutFn, feeExecutor, { value, gasPrice: gasGwei, gasLimit: gaslimitFn });
  // wait confirmation
  ...
  return { id: 1, transactionReceipt: txResponse };
}
...
export async function findOpenOrdersFromTokens(
  readContract: Contract, tokens: TokenMap, chainId?: number
): Promise<IOrder[]> {
  const maxOrders: number = (await readContract.getOrdersLength())?.toNumber() || 0;

  // get orders id
  let promisseOrders: any[] = [];
  for (let i = 0; i < maxOrders; i++) {
    promisseOrders.push(readContract.orders(i));
  }
  ...
  const orders = (await allSettled(promisseOrders))
    .filter(e => e.status === 'fulfilled')
    .map(e => parseEvent((e as { status: string, value: number }).value, tokens, chainId))
    .filter( o => o.state === EOrderState.CREATED )
  return orders
}

export function filterOrdersFromPair(orders: IOrder[], token0?: Currency | Token | IToken, token1?: Currency | Token | IToken){
  if(!token0 || !token1){
    return []
  }
  return orders.filter( o => 
    (
      o.tokenIn.symbol.toLowerCase() === token0?.symbol.toLowerCase() && 
      o.tokenOut.symbol.toLowerCase() === token1?.symbol.toLowerCase()
    ) || (
      o.tokenOut.symbol.toLowerCase() === token0?.symbol.toLowerCase() &&
      o.tokenIn.symbol.toLowerCase() === token1?.symbol.toLowerCase()
    ) ||  ('address' in token0 && 'address' in token1 ? (
      o.tokenIn.address.toLowerCase() === (token0 as (Token | IToken))?.address.toLowerCase() && 
      o.tokenOut.address.toLowerCase() === (token1 as (Token | IToken))?.address.toLowerCase()
    )  || (
      o.tokenOut.address.toLowerCase() === (token0 as (Token | IToken))?.address.toLowerCase() &&
      o.tokenIn.address.toLowerCase() === (token1 as (Token | IToken))?.address.toLowerCase()
    ) : false)
  );
}
