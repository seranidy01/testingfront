...
import { BIG_ZERO, farmsConfig, MAKI_PLS_POOL_PID, MAKI_USD_POOL_PID, USD_PLS_POOL_PID } from 'config'
...
export const usePollCoreFarmData = () => {
  ...
  useEffect(() => {
    dispatch(fetchFarmsPublicDataAsync([MAKI_PLS_POOL_PID, USD_PLS_POOL_PID]))
...
//   const tokenPrice = useUsdPriceFromPid(tokenFarm?.pid)
//   return tokenPrice
// }

export const useLpTokenPrice = (symbol: string) => {
  const farm = useFarmFromLpSymbol(symbol)
  const farmTokenPriceInUsd = useUsdPriceFromPid(farm.pid)
...
// export const usePricePlsHusd = (): BigNumber => {
//   const htHusdFarm = useFarmFromPid(4) // HUSD-HT LP
//   return new BigNumber(htHusdFarm.quoteToken.husdPrice)
// }

export const usePriceMakiUsd = (): BigNumber => {
  const makiUsdFarm = useFarmFromPid(MAKI_USD_POOL_PID) // MAKI-HUSD LP
  return new BigNumber(makiUsdFarm.token.husdPrice)
}
