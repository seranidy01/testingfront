...
import { CurrencyAmount, Currency, JSBI, Token, Trade } from 'maki-pulsechain-sdk'
...
const Limit = () => {
  ...
  const { status, feeStake, feeExecutor, gasPrice, selectedOrder, readContractAddress } = useSelector<AppState,AppState['limit']>((s) => s.limit)
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [price1, formattedAmounts[Field.INPUT]])


  useEffect(() => {
    const makiToken = new Token(BASE_CHAIN_ID, tokens.maki.address[BASE_CHAIN_ID], tokens.maki.decimals, tokens.maki.symbol, tokens.maki.name)
                    ...
                    onClick={() => {
                      if (
                        ...
                        createOrder(
                          ...
                          .catch((e) => {
                            console.error('create order error 3: ', e)
                            dispatch(
                              updateStatus({
                                status: 'failed',
                                error: e?.message,
