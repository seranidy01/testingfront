...
import { ChainId, Pair, Route, TokenAmount, Trade, TradeType, Rounding, Token, WPLS as _WPLS } from '../src'
...
const WPLS = _WPLS[ChainId.TESTNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WPLS, decimalize(1234, WPLS.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WPLS]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WPLS)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WPLS, decimalize(10, WPLS.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WPLS, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WPLS, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WPLS, '1234567000000000000000')
