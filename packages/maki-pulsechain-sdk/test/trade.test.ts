...
import { ChainId, PLS, CurrencyAmount, Pair, Percent, Route, Token, TokenAmount, Trade, TradeType, WPLS } from '../src'
...
describe('Trade', () => {
  ...
  const pair_wht_0 = new Pair(
    new TokenAmount(WPLS[ChainId.MAINNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with PLS as input', () => {
    const trade = new Trade(
      new Route([pair_wht_0], PLS),
      CurrencyAmount.pls(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(PLS)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with PLS as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_wht_0], PLS, token0),
  ...
  it('can be constructed with PLS as output', () => {
    const trade = new Trade(
      new Route([pair_wht_0], token0, PLS),
      CurrencyAmount.pls(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(PLS)
  })
  it('can be constructed with PLS as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_wht_0], token0, PLS),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for PLS currency input', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wht_0, pair_0_1, pair_0_3, pair_1_3],
        CurrencyAmount.pls(JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(PLS)
      expect(result[0].route.path).toEqual([WPLS[ChainId.MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(PLS)
      expect(result[1].route.path).toEqual([WPLS[ChainId.MAINNET], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for PLS currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wht_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WPLS[ChainId.MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(PLS)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WPLS[ChainId.MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(PLS)
    })
  })
