import { ChainId, Token, Pair, TokenAmount, WPLS, Price } from '../src'
...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(USDC, '100'), new TokenAmount(WPLS[ChainId.TESTNET], '100'))).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#getAddress', () => {
    it('returns the correct address', () => {
      expect(Pair.getAddress(USDC, DAI)).toEqual('0xaeB7a68Df4821Be2c1FdD4aC964bB10067157937')
    })
  })
  ...
  describe('#priceOf', () => {
    ...
    it('throws if invalid token', () => {
      expect(() => pair.priceOf(WPLS[ChainId.MAINNET])).toThrow('TOKEN')
    })
  })
  ...
  describe('#reserveOf', () => {
    ...
    it('throws if not in the pair', () => {
      expect(() =>
        new Pair(new TokenAmount(DAI, '101'), new TokenAmount(USDC, '100')).reserveOf(WPLS[ChainId.MAINNET])
      ).toThrow('TOKEN')
    })
  })
  ...
  describe('#involvesToken', () => {
    ...
    expect(
      new Pair(new TokenAmount(USDC, '100'), new TokenAmount(DAI, '100')).involvesToken(WPLS[ChainId.MAINNET])
    ).toEqual(false)
  })
})
